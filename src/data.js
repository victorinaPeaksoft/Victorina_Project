export const data = {
  ru: {
    questions: [
      {
        question: "Ты комфортно чувствуешь себя в одиночестве?",
        answers: [
          {
            variant: "скорее да",
            key: 1,
          },
          {
            variant: "скорее нет",
            key: 2,
          },
          {
            variant: "да",
            key: 3,
          },
          {
            variant: "нет",
            key: 4,
          },
        ],
      },
      {
        question:
          "В тишине ты чувствуешь себя неуютно и стараешься чем-то заполнить ее?",
        answers: [
          {
            variant: "Часто",
            key: 1,
          },
          {
            variant: "Иногда",
            key: 2,
          },
          {
            variant: "Все время",
            key: 3,
          },
          {
            variant: "Никогда",
            key: 4,
          },
        ],
      },
      {
        question: "Ты легко находишь общий язык с незнакомцами?",
        answers: [
          {
            variant: "скорее да",
            key: 1,
          },
          {
            variant: "скорее нет",
            key: 2,
          },
          {
            variant: "да",
            key: 3,
          },
          {
            variant: "нет",
            key: 4,
          },
        ],
      },
      {
        question: "У тебя много знакомых и друзей?",
        answers: [
          {
            variant: "Да",
            key: 1,
          },
          {
            variant: "Скорее да ",
            key: 2,
          },
          {
            variant: "Нет",
            key: 3,
          },
          {
            variant: "Скорее нет",
            key: 4,
          },
        ],
      },
      {
        question: "Тебе сложно усидеть на месте?",
        answers: [
          {
            variant: "Почти всегда",
            key: 1,
          },
          {
            variant: "Иногда",
            key: 2,
          },
          {
            variant: "Все время",
            key: 3,
          },
          {
            variant: "Никогда",
            key: 4,
          },
        ],
      },
      {
        question:
          " Если у вас возникает проблема, решение которой зависит только от вас, расскажете ли вы о ней другу?",
        answers: [
          {
            variant: "Да",
            key: 1,
          },
          {
            variant: "Скорее да",
            key: 2,
          },
          {
            variant: "Скорее нет",
            key: 3,
          },
          {
            variant: "Нет",
            key: 4,
          },
        ],
      },
      {
        question:
          "Хорошо ли вы помните зло, которое вам причинили другие люди?",
        answers: [
          {
            variant: "Иногда",
            key: 1,
          },
          {
            variant: "Да",
            key: 2,
          },
          {
            variant: "Нет",
            key: 3,
          },
          {
            variant: "Никогда",
            key: 4,
          },
        ],
      },
      {
        question: "Вам тяжело быть разговорчивым с новыми знакомыми?",
        answers: [
          {
            variant: "Нет",
            key: 1,
          },
          {
            variant: "Да",
            key: 2,
          },
          {
            variant: "Иногда",
            key: 3,
          },
          {
            variant: "Никогда",
            key: 4,
          },
        ],
      },
      {
        question:
          "Легко ли вам выступать на совещаниях, собраниях, презентациях?",
        answers: [
          {
            variant: "Легко",
            key: 1,
          },
          {
            variant: "Только если хорошо разбираюсь в том, о чем говорю",
            key: 2,
          },
          {
            variant: "Иногда",
            key: 3,
          },
          {
            variant: "Я стараюсь избегать этого",
            key: 4,
          },
        ],
      },
      {
        question: "Как вы обычно воспринимаете критику?",
        answers: [
          {
            variant: "Тяжело",
            key: 1,
          },
          {
            variant: "По разному",
            key: 2,
          },
          {
            variant: "Нормально",
            key: 3,
          },
          {
            variant: "Легко",
            key: 4,
          },
        ],
      },
      {
        question: "Страдаете от бессонницы?",
        answers: [
          {
            variant: "Нет",
            key: 1,
          },
          {
            variant: "Да",
            key: 2,
          },
          {
            variant: "Иногда",
            key: 3,
          },
          {
            variant: "Никогда",
            key: 4,
          },
        ],
      },
      {
        question: "Легко ли вы переносите жизненные неудачи?",
        answers: [
          {
            variant: "Тяжело",
            key: 1,
          },
          {
            variant: "По разному",
            key: 2,
          },
          {
            variant: "Смотря какие",
            key: 3,
          },
          {
            variant: "Легко",
            key: 4,
          },
        ],
      },
      {
        question: "Тяжело ли вам спрашивать что-то у незнакомых людей?",
        answers: [
          {
            variant: "Да",
            key: 1,
          },
          {
            variant: "Иногда тяжело",
            key: 2,
          },
          {
            variant: "Нет",
            key: 3,
          },
          {
            variant: "Легко",
            key: 4,
          },
        ],
      },
      {
        question:
          "Можете ли вы успешно справляться сразу с несколькими задачами?",
        answers: [
          {
            variant: "Да",
            key: 1,
          },
          {
            variant: "Иногда",
            key: 2,
          },
          {
            variant: "Нет",
            key: 3,
          },
          {
            variant: "Смотря с какими",
            key: 4,
          },
        ],
      },
      {
        question: "Вы обычно планируете свои дела заранее?",
        answers: [
          {
            variant: "Да",
            key: 1,
          },
          {
            variant: "По разному",
            key: 2,
          },
          {
            variant: "Только самые важные",
            key: 3,
          },
          {
            variant: "Смотря какие",
            key: 4,
          },
        ],
      },
      {
        question: "Когда на вас кричат, отвечаете ли тем же?",
        answers: [
          {
            variant: "Да",
            key: 1,
          },
          {
            variant: "Иногда",
            key: 2,
          },
          {
            variant: "Скорее нет",
            key: 3,
          },
          {
            variant: "Нет",
            key: 4,
          },
        ],
      },
      {
        question: "Склонны к резким сменам настроения?",
        answers: [
          {
            variant: "Бывает",
            key: 1,
          },
          {
            variant: "скорее нет",
            key: 2,
          },
          {
            variant: "Нет",
            key: 3,
          },
          {
            variant: "Да",
            key: 4,
          },
        ],
      },
      {
        question: "Часто не собранны, проявляете поспешность в решениях?",
        answers: [
          {
            variant: "Скорее да",
            key: 1,
          },
          {
            variant: "Да",
            key: 2,
          },
          {
            variant: "Скорее нет",
            key: 3,
          },
          {
            variant: "Нет",
            key: 4,
          },
        ],
      },
      {
        question: "Любите ли вы быть в центре внимания?",
        answers: [
          {
            variant: "Нет",
            key: 1,
          },
          {
            variant: "Да",
            key: 2,
          },
          {
            variant: "Иногда",
            key: 3,
          },
          {
            variant: "Скорее нет",
            key: 4,
          },
        ],
      },
      {
        question: "Какой тип речи у вас преобладает?",
        answers: [
          {
            variant: "Сдержанный тип",
            key: 1,
          },
          {
            variant:
              "Речь быстрая и сопровождается выразительными мимикой и жестами",
            key: 2,
          },
          {
            variant: "Медленная, со слабой мимикой",
            key: 3,
          },
          {
            variant: "Выразительная мимика, торопливая речь, резкие жесты",
            key: 4,
          },
        ],
      },
      {
        question:
          "Как вы чаще всего чувствуете себя перед важным событием, не имеет значения - хорошим или плохим?",
        answers: [
          {
            variant: "Сильно нервничаю, могу сорваться",
            key: 1,
          },
          {
            variant: "Когда как",
            key: 2,
          },
          {
            variant: "На одной, но глубоко",
            key: 3,
          },
          {
            variant:
              "Остаюсь спокойным или немного нервничаю, но верю, что все будет хорошо",
            key: 4,
          },
        ],
      },
    ],
    victorina: "Тест на темперамент личности",
    temparement: "Узнайте свой темперамент",
    begin: "Начать",
    nextButton: "Следующий",
    prevButton: "Предыдущий",
    submitButton: "Сохранить",
    main: "Ваша результат",
    sanguine: "Сангвик",
    melancholic: "Меланхолик",
    phlegmatic: "Флегматик",
    choleric: "Холерик",
    address: "Главная",
  },
  en: {
    questions: [
      {
        question: "Are you comfortable being alone?",
        answers: [
          {
            variant: "Yes",
            key: 1,
          },
          {
            variant: "Sometimes",
            key: 2,
          },
          {
            variant: "All the time",
            key: 3,
          },
          {
            variant: "Never",
            key: 4,
          },
        ],
      },
      {
        question:
          "In silence, do you feel uncomfortable and try to fill it with something?",
        answers: [
          {
            variant: "Often",
            key: 1,
          },
          {
            variant: "Sometimes",
            key: 2,
          },
          {
            variant: "All the time",
            key: 3,
          },
          {
            variant: "Never",
            key: 4,
          },
        ],
      },
      {
        question: "Do you easily get along with strangers?",
        answers: [
          {
            variant: "Yes",
            key: 1,
          },
          {
            variant: "Sometimes",
            key: 2,
          },
          {
            variant: "All the time",
            key: 3,
          },
          {
            variant: "Never",
            key: 4,
          },
        ],
      },
      {
        question: "Do you have many acquaintances and friends?",
        answers: [
          {
            variant: "Yes",
            key: 1,
          },
          {
            variant: "Sometimes",
            key: 2,
          },
          {
            variant: "All the time",
            key: 3,
          },
          {
            variant: "Never",
            key: 4,
          },
        ],
      },
      {
        question: "Is it difficult for you to sit still?",
        answers: [
          {
            variant: "Always",
            key: 1,
          },
          {
            variant: "Sometimes",
            key: 2,
          },
          {
            variant: "All the time",
            key: 3,
          },
          {
            variant: "Never",
            key: 4,
          },
        ],
      },
      {
        question:
          " If you have a problem that is entirely up to you to solve, will you tell a friend about it?",
        answers: [
          {
            variant: "Yes",
            key: 1,
          },
          {
            variant: "Sometimes",
            key: 2,
          },
          {
            variant: "Allt the time",
            key: 3,
          },
          {
            variant: "Never",
            key: 4,
          },
        ],
      },
      {
        question:
          "Do you remember well the evil that other people have done to you?",
        answers: [
          {
            variant: "Sometimes",
            key: 1,
          },
          {
            variant: "Yes",
            key: 2,
          },
          {
            variant: "No",
            key: 3,
          },
          {
            variant: "Never",
            key: 4,
          },
        ],
      },
      {
        question:
          "Is it difficult for you to be talkative with new acquaintances?",
        answers: [
          {
            variant: "No",
            key: 1,
          },
          {
            variant: "Yes",
            key: 2,
          },
          {
            variant: "Sometimes",
            key: 3,
          },
          {
            variant: "Never",
            key: 4,
          },
        ],
      },
      {
        question:
          "Is it easy for you to speak at meetings, meetings, presentations?",
        answers: [
          {
            variant: "Easily",
            key: 1,
          },
          {
            variant: "Only if I'm well versed in what I'm talking about",
            key: 2,
          },
          {
            variant: "Sometimes",
            key: 3,
          },
          {
            variant: "I try to avoid it",
            key: 4,
          },
        ],
      },
      {
        question: "How do you usually take criticism?",
        answers: [
          {
            variant: "heavy",
            key: 1,
          },
          {
            variant: "Differently",
            key: 2,
          },
          {
            variant: "fine",
            key: 3,
          },
          {
            variant: "Easily",
            key: 4,
          },
        ],
      },
      {
        question: "Are you suffering from insomnia?",
        answers: [
          {
            variant: "No",
            key: 1,
          },
          {
            variant: "Yes",
            key: 2,
          },
          {
            variant: "Sometimes",
            key: 3,
          },
          {
            variant: "Never",
            key: 4,
          },
        ],
      },
      {
        question: "Do you easily tolerate life's failures?",
        answers: [
          {
            variant: "Hard",
            key: 1,
          },
          {
            variant: "Differently",
            key: 2,
          },
          {
            variant: "Depends on what",
            key: 3,
          },
          {
            variant: "easily",
            key: 4,
          },
        ],
      },
      {
        question: "Is it hard for you to ask strangers something?",
        answers: [
          {
            variant: "Yes",
            key: 1,
          },
          {
            variant: "Sometimes it's hard",
            key: 2,
          },
          {
            variant: "No",
            key: 3,
          },
          {
            variant: "easily",
            key: 4,
          },
        ],
      },
      {
        question: "Can you successfully cope with multiple tasks at once?",
        answers: [
          {
            variant: "Yes",
            key: 1,
          },
          {
            variant: "Sometimes",
            key: 2,
          },
          {
            variant: "No",
            key: 3,
          },
          {
            variant: "Depends on what",
            key: 4,
          },
        ],
      },
      {
        question: "Do you usually plan your affairs in advance?",
        answers: [
          {
            variant: "Yes",
            key: 1,
          },
          {
            variant: "Differently",
            key: 2,
          },
          {
            variant: "Only the most important",
            key: 3,
          },
          {
            variant: "Depends on what",
            key: 4,
          },
        ],
      },
      {
        question: "When someone yells at you, do you respond in kind?",
        answers: [
          {
            variant: "Yes",
            key: 1,
          },
          {
            variant: "Sometimes",
            key: 2,
          },
          {
            variant: "probably ",
            key: 3,
          },
          {
            variant: "No",
            key: 4,
          },
        ],
      },
      {
        question: "Are you prone to mood swings?",
        answers: [
          {
            variant: "it happens",
            key: 1,
          },
          {
            variant: "Sometimes",
            key: 2,
          },
          {
            variant: "No",
            key: 3,
          },
          {
            variant: "Yes",
            key: 4,
          },
        ],
      },
      {
        question: "Often not collected, show haste in decisions?",
        answers: [
          {
            variant: "Probably",
            key: 1,
          },
          {
            variant: "Yes",
            key: 2,
          },
          {
            variant: "Never",
            key: 3,
          },
          {
            variant: "No",
            key: 4,
          },
        ],
      },
      {
        question: "Do you love being the center of attention?",
        answers: [
          {
            variant: "No",
            key: 1,
          },
          {
            variant: "Yes",
            key: 2,
          },
          {
            variant: "Sometimes",
            key: 3,
          },
          {
            variant: "Probably",
            key: 4,
          },
        ],
      },
      {
        question: "What type of speech do you predominate?",
        answers: [
          {
            variant: "Discreet type",
            key: 1,
          },
          {
            variant:
              "Speech is fast and is accompanied by expressive facial expressions and gestures",
            key: 2,
          },
          {
            variant: "Slow, with weak facial expressions",
            key: 3,
          },
          {
            variant:
              "Expressive facial expressions, hasty speech, harsh gestures",
            key: 4,
          },
        ],
      },
      {
        question:
          "How do you most often feel before an important event, does it matter - good or bad?",
        answers: [
          {
            variant: "I'm very nervous, I can break",
            key: 1,
          },
          {
            variant: "It depends",
            key: 2,
          },
          {
            variant: "On one but deep",
            key: 3,
          },
          {
            variant:
              "I stay calm or a little nervous, but I believe that everything will be fine",
            key: 4,
          },
        ],
      },
    ],
    victorina: "Personality Temperament Test",
    temparement: "Find out your temperament",
    begin: "Start Test",
    nextButton: "Next",
    prevButton: "Prev",
    submitButton: "Submit",
    main: "Your result",
    sanguine: "Sanguine",
    melancholic: "Melancholic",
    phlegmatic: "Phlegmatic",
    choleric: "Choleric",
    address: "Home",
  },
};
